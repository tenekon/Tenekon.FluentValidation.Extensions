...
using Tenekon.Extensions.FluentValidation.Blazor.Interception.Intent;
...
internal sealed class DescendantFieldIdentifierComparer(EditContext editContext, IAccessLogger accessLogger)
    : IEqualityComparer<FieldIdentifier>
{
    ...
    int IEqualityComparer<FieldIdentifier>.GetHashCode(FieldIdentifier obj)
    {
        ...
        if (!dictionary.Contains(obj)) {
            ...
            var validationMessageStoreEqualityComparer = new DescendantValidationMessageStoreComparer(editContext, obj) {
            ...
            MirroredFieldStateSynchronizer.TryAttachMirroredFieldState(editContext, obj, fieldState, validationMessageStoreHashSet);
