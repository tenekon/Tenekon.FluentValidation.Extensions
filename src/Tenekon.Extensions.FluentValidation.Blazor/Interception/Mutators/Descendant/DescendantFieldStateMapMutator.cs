...
using Tenekon.Extensions.FluentValidation.Blazor.Interception.Accessors;
...
internal sealed class DescendantFieldStateMapMutator(EditContext editContext)
    : IFieldStateMapMutator
{
    ...
    void IFieldStateMapMutator.DoMutation()
    {
        ...
        var fieldIdentifierEqualityComparer = new DescendantFieldIdentifierComparer(editContext, _accessLog);
        var derivedDictionary = CreateMigratedFieldStateMap(dictionary, fieldIdentifierEqualityComparer);
        fieldIdentifierEqualityComparer.Dictionary = derivedDictionary;
        ...
        IEqualityComparer<FieldIdentifier> equalityComparer)
    {
        ...
        migratedDictionary.Add(
            InterceptionWarmupMarkers.FieldIdentifier,
            FieldStateAccessor.Create(InterceptionWarmupMarkers.FieldIdentifier));
        ...
        foreach (DictionaryEntry pair in originalDictionary) {
            ...
            MigrateFieldState(fieldIdentifier, pair.Value);
    ...
    {
        var equalityComparer = new DescendantValidationMessageStoreComparer(editContext, fieldIdentifier) {
            AccessLogger = _accessLog
        };
    ...
    private void MigrateFieldState(FieldIdentifier fieldIdentifier, object fieldState)
    {
        ...
        FieldStateAccessor.SetValidationMessageStores(
            fieldState,
            CreateHookableValidationMessageStoreSet(fieldIdentifier, validationMessageStores));
    }
}
