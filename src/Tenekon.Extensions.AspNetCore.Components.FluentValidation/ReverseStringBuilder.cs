...
internal ref struct ReverseStringBuilder
{
    ...
    public void InsertFront(scoped ReadOnlySpan<char> span)
    {
        ...
        if (_fallbackSequenceSegment is null) {
            ...
            _fallbackSequenceSegment = new SequenceSegment(newBuffer);
            ...
            var newBuffer = s_arrayPool.Rent(sizeToRent);
            _fallbackSequenceSegment = new SequenceSegment(newBuffer, _fallbackSequenceSegment);
            _currentBuffer = newBuffer;
    ...
    public void InsertFront(IFormattable formattable) => InsertFront(formattable.ToString(format: null, CultureInfo.InvariantCulture));

    public override readonly string ToString() =>
        _fallbackSequenceSegment is null ? new string(_currentBuffer[_nextEndIndex..]) : _fallbackSequenceSegment.ToString(_nextEndIndex);

    public readonly void Dispose() => _fallbackSequenceSegment?.Dispose();
